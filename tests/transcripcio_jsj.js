...
'use strict';
/*
 * Guardià de la TRANSCRIPCIÓ JSJ (25/07/2026 · P4 del consell — «Jordi de Sant Jordi»).
 *
...
let api = null;
try {
  // 10/09/2026: el joc d'eines de lectura s'agafa SENCER del fitxer (marques INICI/FI EINES DE LECTURA).
  // Abans es llistava peça a peça i cada ajudant nou deixava el guardià en vermell sense que res estigués trencat.
  const code = [extractFn('_dehyphen'), extractFn('estimateCoverage'), extractFn('majusculesTitols'), extractFn('contracteEstructura'), extractFn('_rangPagines'), (function(){var _i=h.indexOf('// ==== INICI EINES DE LECTURA ===='),_f=h.indexOf('// ==== FI EINES DE LECTURA ====');if(_i<0||_f<0)throw new Error('falten les marques EINES DE LECTURA a app_gta.html');return h.slice(_i,_f);})()].join('\n');
