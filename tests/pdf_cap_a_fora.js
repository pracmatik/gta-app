...
// ── 3 · «amagar línies de càlcul» amaga les de CÀLCUL, i les informatives es queden sense preu (31/07 → 24/09) ──
// La tècnica, 31/07 13:28, literal: «Les línies informatives de preus han desaparegut al amagar les línies de càlcul»
// (com a error), «tornen a tenir els preus unitaris que vam demanar treure» i «algunes de les línies de càlcul no s'amaguen».
// El 31/07 es va llegir el primer punt al revés i s'amagaven TOTES; els seus pressupostos a mà ensenyen el desglossament
// informatiu al document del client i amaguen les línies «*». Corregit el 24/09.
check('en amagar la composició, les informatives es queden (pantalla)', /if\(SHOWSUBS\|\|isV\)html\+=/.test(h));
check('en amagar la composició, les informatives es queden (PDF i camí de reserva)', /if\(ctx\.showSubs!==false\|\|s\.tipo==="subv"\)\{/.test(h) && /if\(ctx\.showSubs!==false\|\|s\.tipo==="subv"\)body\+=/.test(h));
check('una línia informativa mai porta preu al PDF', ((h.match(/s\.tipo!=="subv"&&s\.preu!==""/g) || []).length + (/_calc=s\.tipo==="sub"[^\n]*\n[\s\S]{0,900}?\(_calc&&s\.preu!==""/.test(h) ? 1 : 0)) >= 2, // 24/09: al pdfmake, «_calc» = només la de càlcul
  'la tècnica 31/07: «tornen a tenir els preus unitaris que vam demanar treure»');
