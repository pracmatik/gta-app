...
// 09/09/2026: la porta de la FINESTRA (pestanya) ha desaparegut — els PDF surten per la mateixa porta que l'Excel
// (baixada). Motiu: administració GTA, 09/09 («obre una altra pantalla sense que quedi a la vista…»). Vegeu test_entrega_documents.js.
const H = {
  baixada: troba('function lliuraBaixada('),
  impressio: troba('function lliuraImpressio('),
};
check('A1 · hi ha les dues úniques portes de sortida de documents (baixada i impressió)', !!(H.baixada && H.impressio) && !troba('function lliuraFinestra('));
...
if (!H.baixada || !H.impressio || !cosExp || !cosHist) { console.log('\n' + ok + '/' + (ok + ko) + '  ' + ko + ' MALAMENT'); process.exit(1); }
...
const DECLARADES = [
  ['window.open("","_blank"); ', 1, '09/09/2026: només en queda UNA, dins del motor d\'impressió (printHtmlDoc), al qual només s\'hi arriba per la porta que anota. Els PDF ja no obren cap pestanya.'],
...
for (const [què, re] of CAPACITATS) {
  ...
  while ((m = re.exec(html))) {
    if ([H.baixada, H.impressio].some(h => m.index >= h.ini && m.index < h.fi)) continue;
...
check('A5 · les dues portes anoten l\'exportació',
  /logExport\(/.test(H.baixada.cos) && /logExport\(/.test(H.impressio.cos));
...
let fosc = html; for (const h of [H.baixada, H.impressio]) fosc = fosc.slice(0, h.ini) + ' '.repeat(h.fi - h.ini) + fosc.slice(h.fi);
...
function banc(opts) {
  ...
  new vm.Script([cosHist.cos, cosExp.cos, cosNom ? cosNom.cos : '', H.baixada.cos, H.impressio.cos, cosTot ? cosTot.cos : ''].join('\n')).runInContext(sb);
...
(async () => {
  ...
  const PORTES = [
    ...
    ['pdf', b => b.sb.lliuraBaixada({}, 'OBRA_pressupost.pdf', 'pdf'), 'OBRA_pressupost.pdf'],
    ['carta', b => b.sb.lliuraBaixada({}, 'OBRA_carta.pdf', 'carta'), 'OBRA_carta.pdf'],
    ['fotos', b => b.sb.lliuraBaixada({}, 'OBRA_fotos.pdf', 'fotos'), 'OBRA_fotos.pdf'],
  ...
  // (09/09: els escenaris de la pestanya tancada han desaparegut amb la porta de la finestra)
