...
const HTML_PATH = process.argv[2] || ['app_gta.html','index.html'].map(f => path.join(__dirname,'..',f)).find(f => fs.existsSync(f));
...
// 10/09/2026: les eines de lectura s'agafen SENCERES del fitxer (marques INICI/FI EINES DE LECTURA). Llistades a mà,
// cada ajudant nou deixava aquest guardià en vermell sense que res estigués trencat (10 en vermell el 09/09).
const NAMES = ['_dehyphen', '_stripPageArtifacts'];
const BLOC = (function(){const i=html.indexOf('// ==== INICI EINES DE LECTURA ===='),f=html.indexOf('// ==== FI EINES DE LECTURA ====');if(i<0||f<0)throw new Error('falten les marques EINES DE LECTURA a app_gta.html');return html.slice(i,f);})();
...
vm.createContext(sb);
new vm.Script(NAMES.map(extract).concat([BLOC]).join('\n'), { filename: 'transcripcio_engine.js' }).runInContext(sb);
