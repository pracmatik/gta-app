...
check('PDF-html: preu condicionat a que existeixi', h.includes('const _pT=(s.preu!==""&&s.preu!=null&&isFinite(+s.preu))') || h.includes('const _pT=(s.tipo!=="subv"&&s.preu!==""&&s.preu!=null&&isFinite(+s.preu))')); // 24/09: i una informativa, mai preu
check('pdfmake: preu condicionat a que existeixi', /\{text:\((?:s\.tipo!=="subv"&&|_calc&&)?s\.preu!==""&&s\.preu!=null&&isFinite\(\+s\.preu\)\)\?eurF\(\+s\.preu,idioma\):""/.test(h) && (!/\{text:\(_calc&&s\.preu/.test(h) || /_calc=s\.tipo==="sub"/.test(h))); // 24/09: i una informativa, mai preu (_calc = només la línia de càlcul)
...
check('Excel: la cel·la de preu només s\'escriu si hi ha preu real', /if\((?:_calc&&)?s\.preu!==""&&s\.preu!=null&&isFinite\(\+s\.preu\)\)\{SR\.getCell\(5\)\.value=\+s\.preu;SR\.getCell\(5\)\.numFmt=EUR;\}/.test(h)); // 24/09: i només a la línia de càlcul (una informativa, mai preu, com al PDF)
