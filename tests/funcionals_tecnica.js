...
check('renumberRows fa servir el nº del tècnic (capNumOf) — i des del 09/09 primer el que ja porta la capçalera (r.num: document o tècnic)', /function renumberRows\(rs\)\{ let cap=0,pn=0,capNum="1";/.test(h) && h.includes(':capNumOf(r.desc,cap||1)') && /const _n=String\(r\.num==null\?"":r\.num\)\.trim\(\);capNum=\/\^\\d\+\(\\\.\\d\+\)\*\$\/\.test\(_n\)\?_n:capNumOf/.test(h));
