...
'use strict';
/*
 * Guardià del DESGLOSSAMENT D'AMIDAMENT per zona (22/07/2026 · correu l'administració/la tècnica: el sistema "ja no copia el
...
// 24/09/2026: en FILES sota la partida (lloc · unitat · quantitat, sense preu), com als seus pressupostos a mà — ja no en text
check('mostra el desglòs sota la partida com a FILES (com els seus pressupostos)', /_filesDes=_det\.map\(d=>\(\{tipo:"subv",num:"",desc:d\.lloc,ut:unitOK\(c\.ut\),amid:d\.parcial,preu:"",desglos:true\}\)\)/.test(h) && /_det\.length>=2&&/.test(h) && h.includes('if(_filesDes.length) rows.push(..._filesDes);'));
