...
'use strict';
/*
 * Guardià de la PERSISTÈNCIA DE L'EDICIÓ MANUAL (22/07/2026 · bug la tècnica).
 *
...
// Cas la tècnica: partida amb preu imprès (22) i 2 desglossos manuals que NO quadren amb l'imprès
...
// --- 25/07 P3: el XAT també és EDICIÓ (no importació) — passa false ---------
check('la resposta del xat IA passa false (edició de la feina del tècnic, no importació)',
  h.includes('sanitizeRows(data.rows,false)'));
// --- La importació NO passa false (segueix reconciliant) --------------------
check('exactament 4 crides passen ,false (3 de càrrega + xat; les d\'importació intactes)',
  (h.match(/sanitizeRows\([^;]*,false\)/g) || []).length === 4);
check('la ingesta i la recuperació de fragments segueixen reconciliant (sense false)',
  h.includes('sanitizeRows(rowsUsar)') && h.includes('recovered.concat(sanitizeRows(data.rows)||[])'));
